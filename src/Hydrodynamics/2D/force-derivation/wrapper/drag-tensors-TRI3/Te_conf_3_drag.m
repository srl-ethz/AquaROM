function T = Te_conf_3_drag(A,rho,vw1,vw2,r11,r12,r21,r22,x01,x02,x03,x04,x05,x06,ud1,ud2,ud3,ud4,ud5,ud6)
    T = [32.*A.*rho.*vw1.*(4.*vw1.^2+4.*vw2.^2).^(-3).*(vw1.*(r11.*(ud1+( ...
  ...
  abs(ud2+(-1).*ud6+x02+(-1).*x06).^2).^(-1),0,0,32.*A.*rho.*vw1.*( ...
  ...
  ud2+(-1).*ud6+x02+(-1).*x06).^2).^(-1),32.*A.*rho.*vw2.*(4.* ...
