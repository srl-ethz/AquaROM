% mesh_ABAQUSread_ext
%
% [nodes, elements, nset, elset, nset_name, elset_name] = mesh_ABAQUSread_ext(filename,savemfile)
%
...
%
% Author: Jacopo Marconi, Mathieu Dubied
% Last modified: 13/04/2025

function [nodes, elements, nset, elset, nset_name, elset_name] = mesh_ABAQUSread_ext(filename,savemat)
...
unique_fun_name = ['fun_abaqus2matlab_mesh_', datestr(now,'yyyymmdd_HHMMSSFFF')];
filename_output = [unique_fun_name, '.m'];
...
[nodes, elements, nset, elset, nset_name, elset_name] = feval(unique_fun_name);%fun_abaqus2matlab_mesh();
